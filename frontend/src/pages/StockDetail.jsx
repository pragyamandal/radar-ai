...
import { getTechnicals } from '../api.js';
import {
   CheckCircle2,
   Zap,
   Bell,
...
const StockDetail = () => {
   ...
   const backtest = stockData.backtest || null;
   ...
   useEffect(() => {
      ...
      setPriceChange(null);
      const ticker = displayTicker.toUpperCase().replace('.NS', '') + '.NS';
      getTechnicals(ticker)
         .then(data => {
            if (data && data.current_price) {
               setCurrentPrice(data.current_price);
               setPriceChange(data.position_pct);
            }
         })
         .catch(err => console.error('Price fetch failed for', ticker, err));
   }, [displayTicker]);
                ...
                ========================================= */}
                  <div className="col-span-1 md:col-span-3 flex flex-col gap-8">
                     ...
                     <div className="bg-[#111118] rounded-xl p-6 border-l-4 border-l-blue-500 relative overflow-hidden">
                        ...
                        <p className="text-sm font-medium text-gray-400 mb-6">{displayTicker} Ltd.</p>
                        <div className="flex items-end gap-3">
                           <span className="text-3xl font-black tracking-tight">
                              {currentPrice
                                 ? `₹${Number(currentPrice).toLocaleString('en-IN', { minimumFractionDigits: 1, maximumFractionDigits: 1 })}`
                                 : 'Loading...'}
                           </span>
                           <span className="text-sm font-bold text-emerald-400 mb-1">
                              {priceChange ? `${priceChange}% of 52W` : ''}
                           </span>
                        </div>
                     </div>
                        ...
                        <div className="flex flex-col gap-4">
                           ...
                           <div className="bg-[#111118] border border-white/5 rounded-xl p-5">
                              ...
                              <div className="flex justify-between items-end mb-4">
                                 <span className="text-3xl font-black text-emerald-400 tracking-tight leading-none">{backtest ? backtest.win_rate + '%' : '—'}</span>
                                 <ShieldCheck className="w-6 h-6 text-emerald-400 mb-0.5" />
                              </div>
                              {/* Progress Bar */}
                              <div className="w-full bg-white/5 h-1.5 rounded-full overflow-hidden">
                                 <div className="bg-emerald-400 h-full rounded-full shadow-[0_0_10px_#34d399]" style={{ width: backtest ? backtest.win_rate + '%' : '0%' }}></div>
                              </div>
                           </div>
                           ...
                           <div className="grid grid-cols-2 gap-4">
                              <div className="bg-[#111118] border border-white/5 rounded-xl p-5 flex flex-col justify-center">
                                 <p className="text-[10px] font-bold text-gray-500 tracking-widest uppercase mb-2 leading-tight">BEST<br />RETURN</p>
                                 <span className="text-2xl font-black tracking-tight leading-none mb-2">{backtest ? backtest.best_return + '%' : '—'}</span>
                                 <span className="text-[10px] font-bold text-emerald-400">{backtest ? 'Historical best' : ''}</span>
                              </div>
                              <div className="bg-[#111118] border border-white/5 rounded-xl p-5 flex flex-col justify-center">
                                 <p className="text-[10px] font-bold text-gray-500 tracking-widest uppercase mb-2 leading-tight">WORST<br />RETURN</p>
                                 <span className="text-2xl font-black tracking-tight leading-none text-red-500 mb-2">{backtest ? backtest.worst_return + '%' : '—'}</span>
                           ...
                           <div className="hidden sm:flex bg-white/5 p-1 rounded-lg border border-white/10">
                              {['1H', '1D', '1W', 'ALL'].map(tf => (
                                 <button
                                    ...
                                    className={`px-3 py-1 text-xs font-bold rounded transition-colors ${activeTimeframe === tf
                  ...
                  <div className="col-span-1 md:col-span-3 flex flex-col gap-6">
                     ...
                     <div className="bg-[#111118] border border-white/5 border-t-2 border-t-blue-500 rounded-lg p-6 flex flex-col relative overflow-hidden group">
                        ...
                        <p className="text-sm font-medium leading-relaxed text-gray-300 italic mb-8 relative z-10">
                           {stockData.signal_story
                              ? stockData.signal_story.replace('.NS', '').replace('_', ' ').replace('detected', '— Signal Detected')
                              : displayTicker + ' showing strong technical signals'}
                        </p>
