...
import { getExplanation, getVoiceExplanation, getTechnicals } from '../api.js';
import {
   ChevronLeft,
   Bell,
   User,
...
const WhyPage = () => {
   ...
   useEffect(() => {
      ...
      getTechnicals(displayTicker + '.NS')
         .then(data => setTechnicals(data))
         .catch(err => console.error('Technicals fetch failed:', err));

      const fetchExplanation = async () => {
         try {
            setLoading(true);
            const opportunity = {
               ticker: displayTicker + '.NS',
               signals: stockData.signals || ['volume_spike'],
               signal_story: stockData.signal_story || displayTicker + ' showing strong signals',
               risk_level: stockData.risk_level || 'Moderate',
               backtest: {
                  median_return: stockData.backtest?.median_return ?? 2.86,
                  best_return: stockData.backtest?.best_return ?? 12.5,
                  worst_return: stockData.backtest?.worst_return ?? -4.2,
                  win_rate: stockData.backtest?.win_rate ?? 61.3
               }
            };
            const result = await getExplanation(opportunity);
            if (result && result.explanation) {
               const cleanExplanation = (text) => {
                  ...
                  const cleaned = lines.filter(line => {
                     ...
                     return !lower.includes('explain why') &&
                        !lower.includes('keep it under') &&
                        !lower.includes('use simple english') &&
                        !lower.includes('the data provided') &&
                        !lower.includes('let me structure') &&
                        !lower.includes('draft 1:') &&
                        !lower.includes('draft 2:') &&
                        !lower.includes('word count') &&
                        !lower.includes('let me count') &&
                        !lower.includes('let me interpret');
                  });
                  return cleaned.join('\n').trim();
               };

               setExplanation(cleanExplanation(result.explanation));
            }
         } catch (err) {
   ...
   const handlePlayAudio = async () => {
      ...
      try {
         ...
         const blob = await getVoiceExplanation(text, displayTicker);
               ...
               <header className="flex flex-col mb-10">
                  ...
                  <div>
                     <h1 className="text-5xl font-extrabold tracking-tight mb-4 leading-none">
                        Why I said this <span className="text-gray-600 font-normal">/</span> <span className="text-blue-500">{displayTicker}</span>
                     </h1>
                     <p className="text-lg text-gray-400 max-w-4xl leading-relaxed tracking-wide">
                        {`Analysis generated on ${new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })}. Our AI engine processed real NSE market data including price history, volume patterns, and technical indicators.`}
                     </p>
                  </div>
               </header>
                     ...
                     <div className="flex flex-col gap-12 pl-16 relative">
                        ...
                        <div className="bg-[#111118] border border-white/5 rounded-2xl p-8 relative group">
                           ...
                           <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                              <div className="bg-white/5 border border-white/5 rounded-lg p-4 flex flex-col justify-center">
                                 <span className="text-[9px] font-black uppercase tracking-widest text-gray-500 mb-1">52W POSITION</span>
                                 <span className="text-xl font-black tracking-tight text-emerald-400">{technicals ? `${technicals.position_pct}%` : '+18%'}</span>
                              </div>
                              <div className="bg-white/5 border border-white/5 rounded-lg p-4 flex flex-col justify-center">
                        ...
                        <div className="bg-[#111118] border border-white/5 rounded-2xl p-8 relative group">
                           ...
                           <p className="text-sm text-gray-300 leading-relaxed font-medium">
                              {technicals
                                 ? `Volume analysis shows ${technicals.volume_ratio}x average trading activity. Current price is at ${technicals.position_pct}% of its 52-week range (Low: ₹${technicals.week_52_low?.toLocaleString('en-IN')} — High: ₹${technicals.week_52_high?.toLocaleString('en-IN')}). This unusual volume pattern suggests increased institutional interest.`
                                 : 'Scanning volume and price range data...'}
                           </p>
                        </div>
                        ...
                        <div className="bg-[#111118] border border-white/5 rounded-2xl p-8 relative group">
                           ...
                           <h2 className="text-2xl font-bold tracking-tight mb-4">{technicals && technicals.is_breakout ? '52-Week Breakout Confirmed' : 'Technical Pattern Analysis'}</h2>
                           <p className="text-sm text-gray-300 leading-relaxed font-medium">
                              {technicals ? technicals.breakout_text : 'Analyzing technical patterns...'}
                           </p>
                        </div>
                  ...
                  <div className="md:col-span-4 flex flex-col gap-6">
                     ...
                     <div className="bg-[#1a1c23] border border-white/5 rounded-2xl p-6 group cursor-pointer hover:border-white/10 transition-colors shadow-xl">
                        ...
                        <div className="flex items-center gap-4 mb-6">
                           <div onClick={handlePlayAudio} className="w-14 h-14 rounded-xl bg-blue-500 flex items-center justify-center shadow-[0_5px_20px_rgba(59,130,246,0.3)] group-hover:bg-blue-400 transition-colors shrink-0">
                              <Play className="w-6 h-6 text-white fill-white translate-x-0.5" />
                           </div>
                           <div>
                              <h3 className="text-lg font-bold">{audioLoading ? 'Generating audio...' : 'Synthesis Summary'}</h3>
                     ...
                     <div className="bg-[#111118] border border-white/5 rounded-2xl p-6 mt-4">
                        ...
                        <div className="flex items-end gap-3 mb-6">
                           <span className="text-6xl font-black text-emerald-400 leading-none tracking-tighter">{technicals ? `${technicals.rsi}` : '94'}<span className="text-5xl">%</span></span>
                           <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest pb-1 leading-tight w-20">RSI SCORE</span>
                        </div>
